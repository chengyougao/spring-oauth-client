...
public class OauthServiceImpl implements OauthService {
	...
	@Override
	public String submitAuthorizationCode(String clientIdS, String scope, String clientSecretS,
			String authorization_code_callback) {
		request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
		clientId = clientIdS;
		clientSecret = clientSecretS;
		codeCallback = authorization_code_callback;
		AuthorizationCodeDto codeDto = new AuthorizationCodeDto();
		codeDto.setClientId(clientId);
		codeDto.setRedirectUri(host + codeCallback);
		codeDto.setResponseType("code");
		codeDto.setScope(scope);
	...
	@Override
	public Map<String, String> authorizationCodeCallback(AuthCallbackDto callbackDto) {
		Map<String, String> map = new HashMap<String, String>();
		if (callbackDto.error()) {
			// Server response error
			map.put("message", callbackDto.getError_description());
			map.put("error", callbackDto.getError());
			return map;
			...
			tokenDto.setRedirectUri(host + codeCallback);
			final AccessTokenDto accessTokenDto = retrieveAccessTokenDto(tokenDto);
			if (accessTokenDto.error()) {
				map.put("message", accessTokenDto.getErrorDescription());
				map.put("error", accessTokenDto.getError());
				return map;
			} else {
				String url = unityUserInfoUri + "?access_token=" + accessTokenDto.getAccessToken();
				String unity_user_info = WebUtils.loadJson(url);
				map.put("message", unity_user_info);
				map.put("error", "");
				return map;
			}
		} else {
			// illegal state
			map.put("message", "Illegal \"state\": " + callbackDto.getState());
			map.put("error", "Invalid state");
			return map;
		}
	}
